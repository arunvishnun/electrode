...
`;
    ...
    /**
     *  Remotes are by default async loaded via container.{init, get} api.
     *  For standalone mode (local dev), inject async entry or,
     *  if there is additional signal, then inject async entry e.g. if current app is shell. As this will be loaded first.
     */
    const isBootstrapchunkNeeded = isDev
      ? true
      : xarcOptions.options.subappMode === "shell";

    Fs.writeFileSync(
      Path.join(hmrDir, hmrEntry),
      `
// generated by @aurora/app-dev/config/webpack/partial/entry.js
${
  isBootstrapchunkNeeded
    ? importEntryWithBootStrapChunk
    : importEntryWithoutBootStrapChunk
}

export default subApp;
${isDev ? hmrString(subAppReq) : ""}
`
    );
  ...
  function searchSubApps() {
    ...
    if (_.isEmpty(subApps)) {
      ...
      logger.info(
        `Found version 1 subapps: ${Object.keys(subApps).join(", ")}`
      );
    }

    const isDev = Boolean(process.env.WEBPACK_DEV);
    const hmrDir = Path.resolve(
      xarcCwd,
      AppMode.src.dir,
      isDev ? DEV_HMR_DIR : PROD_DIR
    );
  ...
  function shouldPolyfill() {
    if (xarcOptions.webpack.enableBabelPolyfill) {
      const hasMultipleTarget =
        Object.keys(xarcOptions.babel.envTargets).sort().join(",") !==
        "default,node";
